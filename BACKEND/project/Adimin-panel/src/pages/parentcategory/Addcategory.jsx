...
useEffect(() => {
  if (parms.id && parms.id !== 'undefined') {
    ...
    axios.post(`${import.meta.env.VITE_BASE_URL}${import.meta.env.VITE_CATEGORIES_URL}/detail/${parms.id}`)
      .then((result) => {
        if (result.data._status === true) {
          setcategoriesdetails(result.data._data);
          if(result.data._status == true){
...
const formSubmitHandler = (event) => {
  ...
  if (!updatedid) {
    ...
    axios.post(`${import.meta.env.VITE_BASE_URL}${import.meta.env.VITE_CATEGORIES_URL}/create`, formData)
      .then((result) => {
        if (result.data._status === true) {
    ...
    axios.put(`${import.meta.env.VITE_BASE_URL}${import.meta.env.VITE_CATEGORIES_URL}/update/${parms.id}`, formData)
      .then((result) => {
        if (result.data._status === true) {
          toast.success(result.data._message);
          event.target.reset();
                ...
                <div className="w-2/3">
                  <div className="mb-5">
                    ...
                    <input
                      type="text"
                      name='name'
                      defaultValue={categoriesdetails.name}
                      id="categoryName"
                  ...
                  <div className="mb-5">
                    ...
                    <input
                      type="text"
                      name='order'
                      defaultValue={categoriesdetails.order}
                      id="categoryName"
                      className="text-[19px] border-2 shadow-sm border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block w-full py-2.5 px-3"

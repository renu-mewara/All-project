...
export default function viewcategory() {
  ...
  useEffect(() => {
    axios.post(`${import.meta.env.VITE_BASE_URL}${import.meta.env.VITE_CATEGORIES_URL}/view`,{
      limit: 1,
      page: currentPage,
      name: searchname,
    })
  ...
  const changestatus = ()=>{
    axios.post(`${import.meta.env.VITE_BASE_URL}${import.meta.env.VITE_CATEGORIES_URL}/change-status`,{
      ids : checkboxvalue})
      .then((result) => {
        if (result.data._status == true) {
   ...
   const deleterecord = ()=>{
     axios.put(`${import.meta.env.VITE_BASE_URL}${import.meta.env.VITE_CATEGORIES_URL}/delete`,{
      ids : checkboxvalue
    })
      .then((result) => {
        if (result.data._status == true) {
                      ...
                      <th scope="col" class="p-4">
                        <div class="flex items-center">
                          <input id="checkbox-all-search"
                            onClick={allcheckbox}
                            checked={checkboxvalue.length == categories.length ? 'checked' : ''}

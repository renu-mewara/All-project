...
export default function Sliderview() {
  ...
  const allcheckbox = () => {
    if (checkboxvalue.length == slider.length) {
      setcheckboxvalue([])
    } else {
      setcheckboxvalue([])
      const ids = [];
  ...
  const changestatus = ()=>{
    axios.post(`${import.meta.env.VITE_BASE_URL}${import.meta.env.VITE_SLIDER_URL}/change-status`,{
      ids : checkboxvalue})
      .then((result) => {
        if (result.data._status == true) {
                      ...
                      <th scope="col" class="p-4">
                        <div class="flex items-center">
                          <input id="checkbox-all-search"
                            onClick={allcheckbox}
                            checked={checkboxvalue.length == slider.length ? 'checked' : ''}

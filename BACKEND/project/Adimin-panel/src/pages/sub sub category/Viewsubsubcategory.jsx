...
export default function ViewCategory() {
  ...
  useEffect(() => {
    axios.post(`${import.meta.env.VITE_BASE_URL}${import.meta.env.VITE_SUB_SUB_CATEGORIES_URL}/view`, {
      limit: 10,
      page: currentPage,
      name: searchname,
      parent_categories_id: searchcategory,
      subcategories_id: searchsubcategory
    })
      .then((result) => {
        if (result.data._status == true) {
  ...
  }, [currentPage, searchname, apistatus, searchcategory , searchsubcategory]);
  ...
  const allcheckbox = () => {
    if (checkboxvalue.length == subsubcategories.length) {
      setcheckboxvalue([])
    } else {
      setcheckboxvalue([])
      const ids = [];
  ...
  const changestatus = () => {
    axios.post(`${import.meta.env.VITE_BASE_URL}${import.meta.env.VITE_SUB_SUB_CATEGORIES_URL}/change-status`, {
      ids: checkboxvalue
    })
      .then((result) => {
        if (result.data._status == true) {
  ...
  const deleterecord = () => {
    axios.put(`${import.meta.env.VITE_BASE_URL}${import.meta.env.VITE_SUB_SUB_CATEGORIES_URL}/delete`, {
      ids: checkboxvalue
    })
      .then((result) => {
        if (result.data._status == true) {
                      ...
                      <th scope="col" class="p-4">
                        <div class="flex items-center">
                          <input id="checkbox-all-search"
                            onClick={allcheckbox}
                            checked={checkboxvalue.length == subsubcategories.length ? 'checked' : ''}
                          ...
                          return (
                            <tr class="bg-white  dark:bg-gray-800 dark:border-gray-700 border-gray-200 hover:bg-gray-50 dark:hover:bg-gray-600">
                              ...
                              <th scope="row" class="flex items-center px-6 py-4 text-gray-900 whitespace-nowrap dark:text-white">
                                <div class="py-4">
                                  <div class="text-base font-semibold">{v?.parent_categories?.name || "N/A"}

                                </div>
                                </div>
                              </th>
                               <td class=" py-4">
                                  {v?.sub_categories?.name || "N/A"}                         
                               </td>
                              ...
                              <td class=" py-4">
                                {
                                  v.image
                                    ...
                                    <img className="w-10 h-10 rounded-full"
                                      src={`${import.meta.env.VITE_IMAGE_URL}${v.image}`}
                                    />
                              ...
                              <td class=" py-4">
                                ...
                                <Link to={`/sub sub category/update/${v._id}`} >

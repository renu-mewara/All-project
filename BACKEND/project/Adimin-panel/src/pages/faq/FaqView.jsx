...
export default function Faqview() {
  ...
  }, [currentPage, searchquestion, searchanswer, apistatus]);

  const searchhandler = (event) => {
    event.preventDefault();
    setCurrentPage(1)
    setsearchquestion(event.target.question.value),
      setsearchanswer(event.target.answer.value)


  }
  const filterbyquestion = (event) => {
    setCurrentPage(1)
    setsearchquestion(event.target.value)
  }
  const filterbyanswer = (event) => {
    setCurrentPage(1)
  ...
  const allcheckbox = () => {
    if (checkboxvalue.length == faq.length) {
      setcheckboxvalue([])
    } else {
      setcheckboxvalue([])
      const ids = [];
                      ...
                      <th scope="col" class="p-4">
                        <div class="flex items-center">
                          <input id="checkbox-all-search"
                            onClick={allcheckbox}
                            checked={checkboxvalue.length == faq.length ? 'checked' : ''}
                  ...
                  <tbody>
                    {
                      faq.length > 0
                        ...
                        :
                        <tr class="bg-white dark:bg-gray-800 dark:border-gray-700 border-gray-200 hover:bg-gray-50 dark:hover:bg-gray-600">
                          <td colSpan={5} class="px-6 py-4 text-gray-900 whitespace-nowrap dark:text-white text-center">

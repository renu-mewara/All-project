...
exports.viewsubcategory = async (request, response) => {
    try {
        ...
        await subcategoriesModel.find(filter).select('_id name parent_categories').sort({  _id: 'desc' })
            .then((result) => {
                if (result.length > 0) {
...
exports.create = async(request, response) => {
    ...
    if(request.body.name){
        ...
        data.slug = await generateUniqueSlug(subsubcategoriesModel, slug);
    }
    ...
    try {
        ...
        var saveData = new subsubcategoriesModel(data).save()
            .then(async(result) => {

                await subcategoriesModel.updateOne({ _id: request.body.sub_categories }, { $push: { sub_categories: { $each: [result._id] } } });
...
exports.view = async (request, response) => {
    ...
    try {
        ...
        total_records = await subsubcategoriesModel.find(filter).countDocuments();
        await subsubcategoriesModel.find(filter)
...
try {
    ...
    if (request.file && request.file.name) {
        ...
        var savedata =  await subsubcategoriesModel.updateOne({ _id: request.params.id }, { $set: data }, ) 
...
exports.destroy = async(request, response) => {
    ...
    var data ={
        ...
        var savedata =  await subsubcategoriesModel.updateMany({ _id: request.body.ids }, { $set: data }, ) 
            .then((result) => {
                if (result) {
...
exports.detail = async(request, response) => {
     try {
        await subsubcategoriesModel.findById(request.params.id)
            .then((result) => {
                if (result) {
...
exports.changeStatus = async(request, response) => {
    try {
        var savedata =  await subsubcategoriesModel.updateMany({

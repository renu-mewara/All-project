...
exports.create = async (request, response) => {
    ...
    try {
        var savedata = await new categoriesModel(data).save()
            .then((result) => {
                const data = {
                    _status: true,
                    _message: 'categories created successfully',
                    _data: result
                };
                response.send(data);
            })
...
exports.view = async (request, response) => {
    ...
    try {
        ...
        total_records = await categoriesModel.find(filter).countDocuments();
        await categoriesModel.find(filter).select('name sub_categories image order status')
        .populate('sub_categories','name')
        .skip().limit(limit).sort({  _id: 'desc' })
            .then((result) => {
                if (result.length > 0) {
...
try {
    ...
    if (request.file && request.file.name) {
        ...
        var savedata =  await categoriesModel.updateOne({ _id: request.params.id }, { $set: data }, ) 
            .then((result) => {
...
exports.destroy = async(request, response) => {
    ...
    var data ={
        ...
        var savedata =  await categoriesModel.updateMany({ _id: request.body.ids }, { $set: data }, ) 
            .then((result) => {
                if (result) {
...
exports.changeStatus = async(request, response) => {
    try {
        var savedata =  await categoriesModel.updateMany({

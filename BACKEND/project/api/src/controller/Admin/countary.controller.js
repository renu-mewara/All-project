const countarymodel = require('../../model/countary');
const env = require('dotenv').config();
exports.create = async (request, response) => {
    
    var data = request.body;
    try {
        var savedata = await new countarymodel(data).save()
            .then((result) => {
                const data = {
                    _status: true,
...
exports.view = async (request, response) => {
  try {
    ...
    const total_records = await countarymodel.countDocuments(filter);

    const result = await countarymodel
      .find(filter)
...
try {
    ...
    data.updated_at = Date.now();
        ...
        var savedata =  await countarymodel.updateOne({ _id: request.params.id }, { $set: data }, ) 
            .then((result) => {
...
exports.destroy = async(request, response) => {
    ...
    var data ={
        ...
        var savedata =  await countarymodel.updateMany({ _id: request.body.ids }, { $set: data }, ) 
            .then((result) => {
                if (result) {
                const data = {
                    _status: true,
...
exports.detail = async(request, response) => {
     try {
        await countarymodel.findById(request.params.id)
            .then((result) => {
                if (result) {
...
exports.changeStatus = async(request, response) => {
    try {
        var savedata =  await countarymodel.updateMany({

...
exports.create = async (request, response) => {
    ...
    var data = request.body;
        ...
        data.slug =  await generateUniqueSlug(sliderModel,slug)
    ...
    try {
        var savedata = await new sliderModel(data).save()
            .then((result) => {
                const data = {
                    _status: true,
...
exports.view = async (request, response) => {
    ...
    try {
        ...
        await sliderModel.find(filter).select('name image order status').skip().limit(limit).sort({  _id: 'desc' })
            .then((result) => {
                if (result.length > 0) {
                    ...
                    const data = {
                        _status: true,
                        _message: 'slider found successfully',
                     _paginate: paginate,
...
try {
    ...
    if (request.file && request.file.name) {
        ...
        var savedata =  await sliderModel.updateOne({ _id: request.params.id }, { $set: data }, ) 
            .then((result) => {
...
exports.destroy = async(request, response) => {
    ...
    var data ={
        ...
        var savedata =  await sliderModel.updateMany({ _id: request.body.ids }, { $set: data }, ) 
            .then((result) => {
                if (result) {
